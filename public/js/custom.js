...
$(window).scroll(function(){
	...
	if( b < 700 ){
		$("#nav_button").addClass("invisible");
	} else {
		$("#nav_button").removeClass("invisible");
	}
	...
	$(document).ready(function(){
		...
		$('.disanimated').appear(function() {
			...
			if ( !elem.hasClass(animation) ) {
					setTimeout(function(){
						elem.removeClass( animation + " visible" + "animated" );
					}, 1000);

			} else {
				setTimeout(function(){
					elem.removeClass(" visible" + "animated" );
				}, 1000);
			}
		});
...
function lang_animated (){
	$('.my_animate').appear(function() {
		var elem = $(this);
	...
	*/
	...
	$(document).ready(function(){
		...
		$('#GoToHome, #GoToAbout, #GoToFeatures, #GoToWorks, #GoToTeam, #GoToPricing, #GoToBlog, #GoToContacts, .GoToMap' ).scrollTo({ speed: 1400 });
